 ...
 */
function Calendar_new(beginYear, endYear, language, patternDelimiter, date2StringPattern, string2DatePattern) {
	this.beginYear = beginYear || 1990;
	this.endYear   = endYear   || 2020;
	this.language  = language  || 0;
	this.patternDelimiter = patternDelimiter     || "-";
	this.date2StringPattern = date2StringPattern || Calendar_new.language["date2StringPattern"][this.language].replace(/\-/g, this.patternDelimiter);
	this.string2DatePattern = string2DatePattern || Calendar_new.language["string2DatePattern"][this.language];
...
Calendar_new.prototype.draw = function() {
	calendar = this;
	...
	for(var i = 0; i < 7; i++) {
		...
		_cs[_cs.length] = Calendar_new.language["weeks"][this.language][i];
		_cs[_cs.length] = '<\/th>';	
	}
	_cs[_cs.length] = '<\/tr>';
	for(var i = 0; i < 6; i++){
	...
	this.form.clearButton.value = Calendar_new.language["clear"][this.language];
	this.form.selectTodayButton.value = Calendar_new.language["today"][this.language];
	this.form.closeButton.value = Calendar_new.language["close"][this.language];
...
Calendar_new.prototype.bindYear = function() {
	var ys = this.form.yearSelect;
	ys.length = 0;
	for (var i = this.beginYear; i <= this.endYear; i++){
		ys.options[ys.length] = new Option(i + Calendar_new.language["year"][this.language], i);
	}
};

Calendar_new.prototype.bindMonth = function() {
	var ms = this.form.monthSelect;
	ms.length = 0;
	for (var i = 0; i < 12; i++){
		ms.options[ms.length] = new Option(Calendar_new.language["months"][this.language][i], i);
	}
};

Calendar_new.prototype.goPrevMonth = function(e){
...
Calendar_new.prototype.goNextMonth = function(e){
...
Calendar_new.prototype.changeSelect = function() {
	var ys = this.form.yearSelect;
	var ms = this.form.monthSelect;
...
Calendar_new.prototype.update = function (e){
...
Calendar_new.prototype.bindData = function () {
	var calendar = this;
...
Calendar_new.prototype.getMonthViewDateArray = function (y, m) {
...
Calendar_new.prototype.show = function (dateControl, popuControl) {
...
Calendar_new.prototype.hide = function() {
	this.panel.style.display = "none";
};

Calendar_new.prototype.getElementById = function(id, object){
	object = object || document;
	return document.getElementById ? object.getElementById(id) : document.all(id);
};

Calendar_new.prototype.getElementsByTagName = function(tagName, object){
	object = object || document;
	return document.getElementsByTagName ? object.getElementsByTagName(tagName) : document.all.tags(tagName);
};

Calendar_new.prototype.getAbsPoint = function (e){
	var x = e.offsetLeft;
	var y = e.offsetTop;
...
__ci.document.close();
var calendar1 = new Calendar_new();
//-->