$(document).ready(function(){	//涓�紑濮嬪姞杞芥柟娉�
	...
	autoNode.css("filter","alpha(opacity=50");//閫忔槑搴�
	...
	wordInput.keyup(function(event)//閿洏鎸変笅
	{
	    ...
	    {
	    	var textInput = wordInput.val()+","+resource.val();//鑾峰彇杈撳叆妗嗙殑鍊�
		...
		else if(keyCode ==38 || keyCode ==40)
		{
			...
			if(keyCode == 38)//鍚戜笂
			{
				...
				{
					autoNodes.eq(highlightindex).css("background-color","white");//鏀瑰彉鍘熸潵鑳屾櫙鑹�鐧借壊
			...
			if(keyCode == 40)//鍚戜笅
			{
				if(highlightindex!=-1)//涓嶄负-1 鍘熸潵鏈夎妭鐐硅閫変腑
				{
					autoNodes.eq(highlightindex).css("background-color","white");//鏀瑰彉鍘熸潵閫変腑div鑳屾櫙鑹�
