...
/*     function autocalute() {
	  ...
	  var code_clientname = code.substr(0, 3);
	   var code_sdlbusinessunit= code.substr(5, 2);
	    calute(code_clientname,code_sdlbusinessunit);
	  } */
    ...
    function processResponse() {
     ...
     function getVendorId() {        
       ...
       var clientname2 =  document.getElementById("clientname").value;
       var sdlbusinessunit2 =  document.getElementById("sdlbusinessunit").value;   
       if(clientname2 == ""){
     ...
     //更新菜单函数
     function init() {  
        document.getElementById("ifscwj").value="Y";
        var clientname="<%=qa.getClientname() %>";
        var sdlbusinessunit="<%=qa.getSdlbusinessunit() %>";
        var sdlcostcode="<%=qa.getSdlcostcode() %>";
        var sdlnetworkoffice="<%=qa.getSdlnetworkoffice() %>";
        var sector="<%=qa.getSector() %>";
        var translatorname="<%=qa.getTranslatorname() %>";
        var translationtype="<%=qa.getTranslationtype() %>";
        var reviewername="<%=qa.getReviewername() %>";
                
        var evaluatedtype="<%=qa.getEvaluatedtype() %>";
        var evaluationmethod="<%=qa.getEvaluationmethod() %>";
        

        var sectionevaluated="<%=qa.getSectionevaluated() %>";
        var evaluatorname="<%=qa.getEvaluatorname() %>";
        var qadate="<%=qa.getQadate() %>";
        var  re =/^(\d{4})-(\d{2})-(\d{2})$/; 
     /*    if(! re.test(qadate)) {//判断日期格式符合YYYY-MM-DD标准 
          alert("Date format is error ");
        } */
        
        var componenttype="<%=qa.getComponenttype() %>";
        var sourcelanguage="<%=qa.getSourcelanguage() %>";
        var wordcount="<%=qa.getWordcount() %>";
        var targetlanguage="<%=qa.getTargetlanguage() %>";
        
  
         
		var 	wrongweighted="<%=qa.getWrongweighted() %>";
		var 	wrongcategory="<%=qa.getWrongcategory() %>";
		var 	wrongminor="<%=qa.getWrongminor() %>";
		var 	wrongpoints="<%=qa.getWrongpoints()  %>";
		var 	wrongserious="<%=qa.getWrongserious() %>";
		var 	wrongcritical="<%=qa.getWrongcritical() %>";
		
			
		var 	meaningcategory="<%=qa.getMeaningcategory()  %>";
		var 	meaningminor="<%=qa.getMeaningminor() %>";
		var 	meaningpoints="<%=qa.getMeaningpoints() %>";
		var 	meaningserious="<%=qa.getMeaningserious() %>";
		var 	meaningweighted="<%=qa.getMeaningweighted() %>";
		var 	meaningcritical="<%=qa.getMeaningcritical() %>";
		
		
		
		var 	styleminor="<%=qa.getStyleminor() %>";
		var 	stylepoints="<%=qa.getStylepoints() %>";
		var 	styleserious="<%=qa.getStyleserious() %>";
		var 	styleweighted="<%=qa.getStyleweighted() %>";
		var 	stylecritical="<%=qa.getStylecritical() %>";
		
		
		
		
		var 	omissioncategory="<%=qa.getOmissioncategory() %>";
		var 	omissionminor="<%=qa.getOmissionminor() %>";
		var 	omissionpoints="<%=qa.getOmissionpoints() %>";
		var 	omissionserious	="<%=qa.getOmissionserious() %>";
		var 	omissionweighted="<%=qa.getOmissionweighted() %>";
		var 	omissioncritical="<%=qa.getOmissioncritical() %>";
		
		
		var 	structuralcategory="<%=qa.getStructuralcategory() %>";
		var 	structuralnminor="<%=qa.getStructuralnminor() %>";
		var 	structuralpoints="<%=qa.getStructuralpoints() %>";
		var 	structuralserious="<%=qa.getStructuralserious() %>";
		var 	structuralweighted="<%=qa.getStructuralweighted() %>";
		var 	structuralcritical="<%=qa.getStructuralcritical() %>";
		
		
		
		var 	miscellaneouscategory="<%=qa.getMiscellaneouscategory() %>";
		var 	miscellaneousminor="<%=qa.getMiscellaneousminor() %>";
		var 	miscellaneouspoints="<%=qa.getMiscellaneouspoints() %>";
		var 	miscellaneousserious="<%=qa.getMiscellaneousserious() %>";
		var 	miscellaneousweighted="<%=qa.getMiscellaneousweighted() %>";
		var 	misspellingcategory	="<%=qa.getMisspellingcategory() %>";
		var 	miscellaneouscritical	="<%=qa.getMiscellaneouscritical() %>";
		
		
		var 	misspellingminor="<%=qa.getMisspellingminor() %>";
		var 	misspellingpoints="<%=qa.getMisspellingpoints() %>";
		var 	misspellingserious="<%=qa.getMisspellingserious()%>";
		var 	misspellingweighted="<%=qa.getMisspellingweighted() %>";
		var 	misspellingcritical="<%=qa.getMisspellingcritical() %>";
		
				       
	    var 	punctuationcategory="<%=qa.getPunctuationcategory() %>";
		var 	punctuationminor="<%=qa.getPunctuationminor() %>";
		var 	punctuationpoints="<%=qa.getPunctuationpoints() %>";
		var 	punctuationserious="<%=qa.getPunctuationserious() %>";
	    var 	punctuationweighted="<%=qa.getPunctuationweighted() %>";
	    var 	punctuationcritical="<%=qa.getPunctuationcritical() %>";
	    
	    		    	
			    
        var 	id="<%=qa.getId() %>";
		var 	filecontenttype="<%=qa.getFilecontenttype() %>";
		var 	filefilename="<%=qa.getFilefilename() %>";
		var 	ftpfilename="<%=qa.getFtpfilename() %>";
	    var 	stylecategory="<%=qa.getStylecategory() %>";
	    
	    
	  
	    document.getElementById("id").value =id;
	 	document.getElementById("filecontenttype").value =filecontenttype;
		document.getElementById("filefilename").value =filefilename;
		document.getElementById("ftpfilename").value =ftpfilename;
		    
	    document.getElementById("punctuationcategory").value =punctuationcategory;
	 	document.getElementById("punctuationminor").value =punctuationminor;
		document.getElementById("punctuationpoints").value =punctuationpoints;
		document.getElementById("punctuationserious").value =punctuationserious;
		document.getElementById("punctuationweighted").value =punctuationweighted;
        document.getElementById("punctuationcritical").value =punctuationcritical;
		
		
		document.getElementById("wrongweighted").value =wrongweighted;
		document.getElementById("wrongcategory").value =wrongcategory;
		document.getElementById("wrongminor").value =wrongminor;
		document.getElementById("wrongpoints").value =wrongpoints ;
		document.getElementById("wrongserious").value =wrongserious;
		document.getElementById("wrongcritical").value =wrongcritical;
		
		
		document.getElementById("meaningcategory").value =meaningcategory;
		document.getElementById("meaningminor").value =meaningminor;
		document.getElementById("meaningpoints").value =meaningpoints;
		document.getElementById("meaningserious").value =meaningserious;
		document.getElementById("meaningweighted").value =meaningweighted;
		document.getElementById("meaningcritical").value =meaningcritical;
		
				
		document.getElementById("stylecategory").value =stylecategory;
		document.getElementById("styleminor").value =styleminor;
		document.getElementById("stylepoints").value =stylepoints;
		document.getElementById("styleserious").value =styleserious;
		document.getElementById("styleweighted").value =styleweighted;
		document.getElementById("stylecritical").value =stylecritical;
		
		
		document.getElementById("omissioncategory").value =omissioncategory;
		document.getElementById("omissionminor").value =omissionminor;
		document.getElementById("omissionpoints").value =omissionpoints;
		document.getElementById("omissionserious").value =omissionserious;
		document.getElementById("omissionweighted").value =omissionweighted;
		document.getElementById("omissioncritical").value =omissioncritical;
		
		
		document.getElementById("structuralcategory").value =structuralcategory;
		document.getElementById("structuralnminor").value =structuralnminor;
		document.getElementById("structuralpoints").value =structuralpoints; 
		document.getElementById("structuralserious").value =structuralserious;
		document.getElementById("structuralweighted").value =structuralweighted;
		document.getElementById("structuralcritical").value =structuralcritical;
		
		
		
		document.getElementById("miscellaneouscategory").value =miscellaneouscategory;
		document.getElementById("miscellaneousminor").value =miscellaneousminor;
		document.getElementById("miscellaneouspoints").value =miscellaneouspoints;
		document.getElementById("miscellaneousserious").value =miscellaneousserious;
		document.getElementById("miscellaneousweighted").value =miscellaneousweighted;
		document.getElementById("miscellaneouscritical").value =miscellaneouscritical;
		
		
		
		document.getElementById("misspellingcategory").value =misspellingcategory;
		document.getElementById("misspellingminor").value =misspellingminor;
		document.getElementById("misspellingpoints").value =misspellingpoints;
		document.getElementById("misspellingserious").value =misspellingserious;
		document.getElementById("misspellingweighted").value =misspellingweighted;
		document.getElementById("misspellingcritical").value =misspellingcritical;

        var adequacylevel="<%=qa.getAdequacylevel() %>";
        var qastatus="<%=qa.getQastatus() %>";
        var sdlqualityindicator="<%=qa.getSdlqualityindicator() %>";
        
        var legacydataerrors="<%=qa.getLegacydataerrors() %>";
        var sourcetexterrors="<%=qa.getSourcetexterrors() %>";
        
        var errorpointsallowed="<%=qa.getErrorpointsallowed()  %>";
        var totalcategoryerrorpoints="<%=qa.getTotalcategoryerrorpoints() %>";
        var qualityindicator="<%=qa.getQualityindicator() %>";
        var sdlqualityinfor="<%=qa.getSdlqualityinfor() %>";
        
        
        var comment="<%=qa.getComment() %>";
        
        
        var  evaluatedtype="<%=qa.getEvaluatedtype() %>";
	    var sel_evaluatedtype=document.getElementById("evaluatedtype");
        for(var i=0;i<sel_evaluatedtype.options.length;i++)
              {
  	       if(sel_evaluatedtype.options[i].text==evaluatedtype)
  	         {
  	           sel_evaluatedtype.options[i].selected=true;
  	           break;
  	         }
           }
	    ...
	    var evaluationmethod="<%=qa.getEvaluationmethod() %>";
	    var sel_evaluationmethod=document.getElementById("evaluationmethod");
        for(var i=0;i<sel_evaluationmethod.options.length;i++)
              {
  	            if(sel_evaluationmethod.options[i].text==evaluationmethod)
  	          {
  	           sel_evaluationmethod.options[i].selected=true;
  	           break;
  	       }
         }
        ...
        document.getElementById("clientname").value =clientname;
        document.getElementById("sdlbusinessunit").value =sdlbusinessunit;
        document.getElementById("costcode").value =sdlcostcode;
        document.getElementById("sdlnetworkoffice").value =sdlnetworkoffice;
        document.getElementById("sdlqualityinfor").value =sdlqualityinfor;
        
       
        var sel_sdlnetworkoffice=document.getElementById('sdlnetworkoffice');
        for(var i=0;i<sel_sdlnetworkoffice.options.length;i++)
              {
  	       if(sel_sdlnetworkoffice.options[i].text==sdlnetworkoffice)
  	         {
  	           sel_sdlnetworkoffice.options[i].selected=true;
  	       break;
  	       }
           }

           var sel_qastatus=document.getElementById('qastatus');
           for(var i=0;i<sel_qastatus.options.length;i++)
              {
  	       if(sel_qastatus.options[i].text==qastatus)
  	         {
  	           sel_qastatus.options[i].selected=true;
  	           break;
  	       }
           }
           
          //sdlqualityindicator=sdlqualityindicator.toFixed(1) 
          document.getElementById("sdlqualityindicator").value = sdlqualityindicator;
          if(sdlqualityindicator >=4 ) {
	      document.getElementById("sdlqualityindicator_show").value = "Pass";
	      }else if(sdlqualityindicator<4) {
	          document.getElementById("sdlqualityindicator_show").value = "No Pass";
	      }else {
	        alert("The format is not correct");
	      }
	      
           var sel_sector=document.getElementById('sector');
           for(var i=0;i<sel_sector.options.length;i++)
              {
  	       if(sel_sector.options[i].text==sector)
  	         {
  	       sel_sector.options[i].selected=true;
  	       break;
  	       }
           }

           document.getElementById("translatorname").value =translatorname;
                      
           var sel_translationtype=document.getElementById('translationtype');
           for(var i=0;i<sel_translationtype.options.length;i++)
              {
  	       if(sel_translationtype.options[i].text==translationtype)
  	         {
  	       sel_translationtype.options[i].selected=true;
  	       break;
  	       }
           }
           ...
           document.getElementById("sectionevaluated").value =sectionevaluated;
           document.getElementById("evaluatorname").value =evaluatorname;
           document.getElementById("qadate").value =qadate;
           
           document.getElementById("legacydataerrors").value =legacydataerrors;
           document.getElementById("sourcetexterrors").value =sourcetexterrors;
           document.getElementById("errorpointsallowed").value =errorpointsallowed;
           document.getElementById("totalcategoryerrorpoints").value =totalcategoryerrorpoints;
           qualityindicator=100*qualityindicator;
           qualityindicator=qualityindicator.toFixed(2)+"%";
           
           document.getElementById("qualityindicator").value =qualityindicator;           
           document.getElementById("comment").value =comment;
           
           
           var sel_componenttype=document.getElementById('componenttype');
           for(var i=0;i<sel_componenttype.options.length;i++)
              {
  	       if(sel_componenttype.options[i].text==componenttype)
  	         {
  	       sel_componenttype.options[i].selected=true;
  	       break;
  	       }
           }
                      
                    
           var sel_sourcelanguage=document.getElementById('sourcelanguage');
           for(var i=0;i<sel_sourcelanguage.options.length;i++)
              {
  	       if(sel_sourcelanguage.options[i].text==sourcelanguage)
  	         {
  	           sel_sourcelanguage.options[i].selected=true;
  	       break;
  	       }
           }
           ...
           //document.getElementById("adequacylevel").value =adequacylevel;
           
           var sel_adequacylevel=document.getElementById('adequacylevel');
           for(var i=0;i<sel_adequacylevel.options.length;i++)
              {
  	       if(sel_adequacylevel.options[i].text==adequacylevel)
  	         {
  	       sel_adequacylevel.options[i].selected=true;
  	       break;
  	       }
           }
           //document.getElementById("targetlanguage").value =targetlanguage;
           var sel_targetlanguage=document.getElementById('targetlanguage');
           
           for(var i=0;i<sel_targetlanguage.options.length;i++)
              {
  	       if(sel_targetlanguage.options[i].text==targetlanguage)
  	         {
  	           sel_targetlanguage.options[i].selected=true;
  	       break;
  	       }
           }
...
/*         // 创建级联菜单函数
     function autocalute() {
         ...
         var clientname = sdlcostcode.substr(0, 3);
         var sdlbusinessunit = sdlcostcode.substr(5, 2);
         sendRequest("calute.jsp?code_clientname="+clientname+"&code_sdlbusinessunit="+sdlbusinessunit);       
    } */
   ...
   /*      fwbz = "vendorid";   
       ...
       if(word== "" || word== null )
       {
    	     ...
    	     return false; 
                ...
                sendRequest("getVendorId.jsp?vendorname=" + word);       
        }  */
       fwbz = "vendorid"; 
       var sdlbusinessunit=""; 
	   var word = encodeURI(document.getElementById("word").value);
...
function test() {
        document.getElementById("Hidden1").value = "1"; 
         ...
         if(clientname=="" || clientname==null || sdlbusinessunit== undefined ) 
          {
    ...
    function ResetAddQa(){
         if (confirm("Do you want to overwrite the date with new QA ?")) {
...
<body onload="init()">  
         ...
         <div class="addform">                
          ...
          <form action="addqa.action" method="post"  onsubmit="return test();">   
           <table width="94%" align="center" border="1" >  
                               
                 <tr style="display:none">  
                 <td align="center" ><input type="text" id="id"     name="id"   "   style="width:90%;height:20px;font-size: 11px;background:#E0E0E0"  ></td>
                 <td align="center" ><input type="text" id="filecontenttype"     name="filecontenttype"      readonly="readonly"   style="width:90%;height:20px;font-size: 11px;background:#E0E0E0"  ></td>
                 <td align="center" ><input type="text" id="filefilename"     name="filefilename"   readonly="readonly"   style="width:90%;height:20px;font-size: 11px;background:#E0E0E0"  ></td>
                 <td align="center" ><input type="text" id="ftpfilename"     name="ftpfilename"     readonly="readonly"   style="width:90%;height:20px;font-size: 11px;background:#E0E0E0"  ></td>
         </tr> 
           
                  <tr>  
                 <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>Vendor Name</span></td>
                 <td>
               <input type="text" id="word"     name="word"   style="width:90%;height:20px;font-size: 11px;"  ><span class='redtext'>*</span><div id = "auto"> </div>
                 </td>
                 
                  <td style="display:none">
               <input type="text" id="lrr"     name="lrr"  value="<%=session.getAttribute("username") %>"  style="width:90%;height:20px;font-size: 11px;"  ><span class='redtext'>*</span><div id = "auto"> </div>
                 </td>
                 
                 
               <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Vendor ID</span></td>
                 <td><input type="text" id="vendorid"     name="vendorid"  onclick="showVendorId()"   readonly="readonly"   style="width:90%;height:20px;font-size: 11px;background:#E0E0E0"  ><span class='redtext'>*</span></td>
               
                 </tr>
                 <tr>  
                 <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>Lead</span></td>
                 <td><input type="text" id="lead"     name="lead"   value="<%=session.getAttribute("username") %>"   readonly="readonly"  style="width:90%;height:20px;font-size: 11px;background:#E0E0E0"   ><span class='redtext'>*</span></td>
                 <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>LM/GL</span></td>
                 <td><input type="text" id="lmorgl"     name="lmorgl"   value="<%=session.getAttribute("lmorgl") %>"   readonly="readonly"    style="width:90%;height:20px;font-size: 11px;background:#E0E0E0"   ><span class='redtext'>*</span></td>
                 </tr> 
                 <tr>  
                 <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>100% matches</span></td>
                 
                 <td>
                     <select id ="matches" name="matches" style="width:90%;height:20px;font-size: 11px;font-size:11px;">
                     ...
                     <td align="center" height="25px" ><span class='title'>&nbsp;</span></td>
                     <td> &nbsp;</td> 
                 </tr> 
                ...
                <tr>
                 ...
                 <td align="center" colspan="4" bgcolor='#e5f1f4' height="25px" ><span class='title'>&nbsp;</span></td>
               
                 </tr> 
                 
                 
                    <tr>  
                    <td width="20%" align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Client Name</span></td>
                    <td width="30%"><input id="clientname" name="clientname" type="text"   readonly="readonly"  style="width:90%;height:20px;font-size: 11px;background:#E0E0E0;"   /><span class='redtext'>*</span></td>
                    <td width="20%" align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>SDL Business Unit</span></td>
                     <td  width="30%"><input  type="text" id="sdlbusinessunit"   name="sdlbusinessunit" readonly="readonly"  style="width:90%;height:20px;font-size: 11px;background:#E0E0E0;" /><span class='redtext'>*</span></td>
                </tr> 
                   ...
                   <tr>  
                 <td align="center"  style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>SDL Cost Code</span></td>
                 <td><input type="text" id="costcode"     name="sdlcostcode"  style="width:90%;height:20px;font-size: 11px;background:#E0E0E0;"  ><span class='redtext'>*</span></td>
                 <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>SDL Lanaguage Office</span></td>
                 <td>
                     <select id ="sdlnetworkoffice" name="sdlnetworkoffice" style="width:90%;height:20px;font-size: 11px;font-size:11px;">
                 ...
                 <tr>  
                 <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Sector</span></td>
                 <td>
                   <select id ="sector" name="sector" style="width:90%;height:20px;font-size: 11px;font-size:11px;">
                 ...
                 <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>Translator Name</span></td>
                 <td><input type="text" id="translatorname"  name="translatorname" data-beatpicker="true" style="width:90%;height:20px;font-size: 11px;"  ><span class='redtext'>*</span></td>
                 </tr>
                 <tr>  
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>Translation Type</span></td>
                    <td>
                    <select id="translationtype" name="translationtype" style="width:90%;height:20px;font-size: 11px;font-size:11px;">
						<option value="Conventional">Conventional</option>
						<option value="i-MTpost-edit">i-MT post-edit</option>
						</select>
                    <span class='redtext'>*</span></td>
                   
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'  ><span class='title'>Reviewer Name</span></td>
                    <td><input type="text" id="reviewername"  name="reviewername" style="width:90%;height:20px;font-size: 11px;"  ><span class='redtext'>*</span></td>
                 </tr>
                 
                              
               
                     <tr>  
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Section Evaluated</span></td>
                    <td><input type="text" id="sectionevaluated"     name="sectionevaluated" style="width:90%;height:20px;font-size: 11px;"  ></td>
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'  ><span class='title'>Evaluator name</span></td>
                    <td><input type="text" id="evaluatorname"  name="evaluatorname" style="width:90%;height:20px;font-size: 11px;"  ><span class='redtext'>*</span></td>
                 </tr>
           ...
           <tr>  
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Date</span></td>
                    <td><input type="text" id="qadate"     name="qadate" onclick="new Calendar_new(null, null, 1).show(this);" readonly="readonly" style="width:90%;height:20px;font-size: 11px;"  ><span class='redtext'>*</span></td>
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'  ><span class='title'>Component Type</span></td>
                   <td>
                    <select id="componenttype" name="componenttype" style="width:90%;height:20px;font-size: 11px;font-size:11px;">
                    ...
                    <span class='redtext'>*</span></td>
                        ...
                        <tr>  
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Source Language</span></td>
                                   <td>
                    <select id="sourcelanguage" name="sourcelanguage" style="width:90%;height:20px;font-size: 11px;font-size:11px;">
                    ...
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'  ><span class='title'>Word Count</span></td>
                    <td><input type="text" id="wordcount"  name="wordcount" style="width:90%;height:20px;font-size: 11px;"  ><span class='redtext'>*</span></td>
                 </tr>
                           <tr>  
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Target Language</span></td>
                    <td>
                    <select id="targetlanguage" name="targetlanguage" style="width:90%;height:20px;font-size: 11px;font-size:11px;">
                    					<option value="Chinese - Simplified">Chinese - Simplified</option>
				    <option value="Acholi">Acholi</option>
					<option value="Afrikaans">Afrikaans</option>
					<option value="Akan">Akan</option>
                    ...
                    <td align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'  ><span class='title'>Adequacy Level</span></td>
                    <td><select id="adequacylevel" name="adequacylevel" style="width:90%;height:20px;font-size: 11px;font-size:11px;">
               ...
               <table width="94%" align="center" border="1" cellspacing="0" cellpadding="0" bordercolor=#000000 style="border-collapse:collapse;border-top:none;">  
                 ...
                 <tr>  
                    <td  align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Error Category</span></td>
             ...
             <tr>  
              ...
              <tr>  
                ...
                <tr>  
                    <td  align="center" colspan="3" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Legacy Data Errors</span></td>
                    <td  align="center" ><span class='title'><input type="text" id="legacydataerrors"     name="legacydataerrors" style="width:90%;height:20px;font-size: 11px;"  ></span></td>
                    <td  align="center" colspan="3" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>Source Text Errors</span></td>
                    <td  align="center" ><span class='title'><input type="text" id="sourcetexterrors"     name="sourcetexterrors" style="width:90%;height:20px;font-size: 11px;"  ></span></td>
                </tr> 
                 ...
                 <tr>  
                    <td  align="center" colspan="5"  style="height:25px;font-size: 11px;" bgcolor='#e5f1f4' ><span class='title'>Quality Scoring Matrix</span></td>
                </tr> 
                <tr>  
                    <td  align="center" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4'><span class='title'>Error Points Allowed</span></td>
                    <td  align="center" ><span class='title'>Total Category Error Points</span></td>
                    <td  align="center" ><span class='title'>% Quality Indicator (100% max)</span></td>
                </tr> 
                <tr>  
                    <td  align="center" style="height:25px;font-size: 11px;"><span class='title'><input type="text" id="errorpointsallowed"     name="errorpointsallowed" style="width:90%;height:20px;font-size: 11px;"  ></span> <span class='redtext'>*</span></td>
                    <td  align="center" ><span class='title'><input type="text" id="totalcategoryerrorpoints"  name="totalcategoryerrorpoints" style="width:90%;height:20px;font-size: 11px;"  ></span> <span class='redtext'>*</span></td>
                    <td  align="center" ><span class='title'><input type="text" id="qualityindicator"     name="qualityindicator" style="width:90%;height:20px;font-size: 11px;"  ></span> <span class='redtext'>*</span></td>
                </tr> 
                  <tr>  
                    <td  align="center" colspan="5" bgcolor='#e5f1f4'  style="height:25px;font-size: 11px;" ><span class='title'><input type="text" id="sdlqualityindicator_show"     name="sdlqualityindicator_show" readonly="readonly" style="width:60px;height:20px;font-size: 11px;background:#E0E0E0" readonly="readonly"  >
                    &nbsp;&nbsp;<input type="text" id="sdlqualityindicator"     name="sdlqualityindicator" onBlur="calutesdlqualityindicator()" style="height:20px" >&nbsp;&nbsp;SDL Quality Indicator						
                    </span></td>
                </tr> 
                 ...
                 <tr>  
                    <td align="center" colspan="5"><input type="text" id="sdlqualityinfor"     name="sdlqualityinfor" style="width:90%;height:30px"></td>
                         ...
                         <tr>  
                    <td  align="center" colspan="5" style="height:25px;font-size: 11px;"     bgcolor='#e5f1f4' ><span class='title'>Instructions:	Enter error details in the 'Evaluation details_input' sheet (yellow tab).									
	See 'Errors & Weighting Definition' sheet for explanation of error categories.									
                         ...
                         <tr>  
                    <td  align="center" colspan="2" height="25px" bgcolor='#e5f1f4'><span class='title'>OVERALL READABILITY & COMMENTS   &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;Select grade for overall Readability >>></span></td>
                    <td  align="center"  colspan="3"  ><span class='title'><select id="qastatus" name="qastatus" style="width:90%;height:25px;font-size: 11px;font-size:11px;">
     ...
     <td colspan="4" ><span class='title'></span><textarea input type="text" id="comment" name="comment" style="width:100%;height:100px;"  > </textarea></td>
      ...
      <center><input type="submit" value="Save" class="buttonstyle" style="width:60px;height:25px;font-size: 11px;" /> <input  type="button" value="Reset"  onclick="ResetAddQa();" class="buttonstyle"  style="width:60px;height:25px;font-size: 11px;" /><input id="Hidden1" type="hidden" /> 
     <!--  <input type="file" id="upfile" name="upload"  />
      <input type="button" onclick="ReadExcel();" value="import">  -->
      </center>
